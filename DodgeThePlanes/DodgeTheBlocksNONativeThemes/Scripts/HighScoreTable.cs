...
public class HighScoreTable : MonoBehaviour
{
    ...
    private Color lightThemeBackground = new Color(222 / 255f, 222 / 255f, 222 / 255f);
    ...
    private void CreateHighscoreEntryTransform(GameDataManager.HighscoreEntry highscoreEntry, Transform container, List<Transform> transformList)
    {
        ...
        if(rank <= 10)
        {
            ...
            entryTransform.Find("Rank (TMP)").GetComponent<TextMeshProUGUI>().text = rankString;

            int score = highscoreEntry.score;
            entryTransform.Find("Score (TMP)").GetComponent<TextMeshProUGUI>().text = score.ToString();
            string date = highscoreEntry.date;
            entryTransform.Find("Date (TMP)").GetComponent<TextMeshProUGUI>().text = date;
