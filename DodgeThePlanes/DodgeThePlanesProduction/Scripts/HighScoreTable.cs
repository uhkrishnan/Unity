...
public class HighScoreTable : MonoBehaviour
{
    ...
    private void CreateHighscoreEntryTransform(GameDataManager.HighscoreEntry highscoreEntry, Transform container, List<Transform> transformList)
    {
        ...
        if(rank <= 10)
        {
            ...
            entryTransform.Find("Rank (TMP)").GetComponent<Text>().text = rankString;
            int score = highscoreEntry.score;
            entryTransform.Find("Score (TMP)").GetComponent<Text>().text = score.ToString();
            string date = highscoreEntry.date;
            entryTransform.Find("Date (TMP)").GetComponent<Text>().text = date;
