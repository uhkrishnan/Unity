...
public class LineIndicator : MonoBehaviour
{
    ...
    private (int, int) GerSquarePosition(int square_index)
    {
        int pos_row = -1;
        int pos_col = -1;
    ...
    {
        int[] line = new int[9];
        var square_pos_row = GerSquarePosition(square_index).Item1;

        for (int index = 0; index < 9; index++)
        {
            line[index] = line_data[square_pos_row, index];
        }

        return line;
    }

    public int[] GerVerticalLine(int square_index)
    {
        int[] line = new int[9];
        var square_pos_col = GerSquarePosition(square_index).Item2;
